...
def main(args):
    ...
    launchpad = Launchpad.login_with('openstack-ansible-release',
                                     options.lpsystem)
    ...
    if options.dry_run:
        ...
        if committed_tasks:
            if not options.force:
                answer = raw_input("Mark these %s bugs as Fix Released?  [y/N]"
                        % len(committed_tasks))
            ...
            print "There are no tasks in %s state, exiting..." % FIX_COMMITTED
            sys.exit(0)
